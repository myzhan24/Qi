...
public class Stage extends GameState{
	...
	public Stage(GameStateManager gsm)
	{
		this.gsm = gsm;
		init();
	}
	...
	public void init() {
		...
		tileMap.loadMap("/Maps/tmap1.map");
		tileMap.setPosition(0,0);
		
		bg = new Background("/Backgrounds/creek.gif",0.1);
		
		player = new Champion(tileMap);
		...
		bgm = new AudioPlayer("/SFX/EB/wound.wav");
	...
	private void populateEnemies()
	{
		...
		for(Point p : derp)
		{
			s = new Slugger(tileMap,Math.random()*0.1+0.05, Math.random()*0.3+0.1);
	...
	public void keyPressed(int k) {
		...
		if(k == KeyEvent.VK_F) player.setPunchCharging();
		if(k == KeyEvent.VK_Q) player.setDashing();
		if(k == KeyEvent.VK_A) player.setTaunting();
